...
Notice that we changed =b= by appending =6=, yet the value of =a= is
...
The logic above explains why the =b= along with =a=, but why didn't the
[[mutint-ex][example]] with integers behave the same way? This has to do with an
...
#+attr_html: :align center :class table-striped
| Type   | Mutable |
|--------+---------|
| int    | no      |
| string | no      |
